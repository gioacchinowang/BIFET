...
template class Frame_freespectral<1>;
template class Frame_freespectral<2>;
template class Frame_freespectral<3>;

template <int dim>
Frame_freespectral<dim>::Frame_freespectral(const Param *par) {
  ...
  case 1:
    ...
    this->pivot_max = dealii::Point<dim, double>(par->grid_set.q2_max);
...
template <int dim> void Frame_freespectral<dim>::init() {
...
template <int dim>
void Frame_freespectral<dim>::refine(
    const dealii::Vector<float> *err_per_cell) {
  if (err_per_cell) {
